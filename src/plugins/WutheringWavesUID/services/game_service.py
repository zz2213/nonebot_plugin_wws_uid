...
from ..core.api.ranking_api import (
    ...
    SlashAppearRateRequest, SlashAppearRateResponse,
    PoolResponse
)
...
class GameService:
    ...
    async def get_period_info(self, cookie: str, uid: str) -> Dict[str, Any]:
        """获取资源统计 (来自游戏API)"""
        cache_key = f"period_info_{uid}"
        cached = await get_cache(cache_key)
        if cached:
            return json.loads(cached)
        result = await waves_api.get_period_info(cookie, uid)
        if result.success:
            await set_cache(cache_key, json.dumps(result.data), 1800)
            return result.data
        return {}
    ...
    async def get_pool_list(self) -> Optional[PoolResponse]:
        """获取卡池统计 (来自排行API)"""
        cache_key = "pool_list_data"
        cached = await get_cache(cache_key)
        if cached:
            return PoolResponse(**json.loads(cached))
        result = await ranking_api.get_pool_list()
        if result.code == 200 and result.data:
            await set_cache(cache_key, result.json(), 3600 * 6)
            return result
        return None
