...
class GameService:
    ...
    async def get_total_rank(self, total_rank_request: TotalRankRequest) -> Optional[TotalRankResponse]:
        ...
        if result.code == 200 and result.data:
            await set_cache(cache_key, result.json(), 1800) # 缓存30分钟
            return result
        return None
    ...
    async def upload_panel_data(self, cookie: str, uid: str) -> str:
        ...
        # 这里你需要根据 `UPLOAD_URL` 的实际需要来构建上传的数据
        # `WutheringWavesUID1/wutheringwaves_master/__init__.py` 的 `upload_waves_info` 函数有具体实现
        # 暂时用 role_data 作为示例
        upload_data = role_data # 
